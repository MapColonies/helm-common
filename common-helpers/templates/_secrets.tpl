...
{{/*
Returns a secret metadata property by secret name and property key from a global secrets metadata array

Usage:
{{ include "common.secrets.getSecretByName" (dict "name" "secret-name" "key" "key-name" "context" $) }}

Params:
  - name - String - Required - Name of a 'Secret' resource.
  - key - String - Required - Name of the property to return.
  - context - Context - Required - Parent context.
*/}}
{{- define "common.secrets.getSecretByName" -}}
{{- $secrets := (include "common.tplvalues.getGlobalObject" (dict "objName" "secrets" "context" .context)) | fromYamlArray -}}
{{- $name := .name -}}
{{- $key := .key -}}
{{- range $secrets -}}
  {{- if eq .name $name -}}
...
{{/*
Returns tls secret name

Usage:
{{ include "common.secrets.tlsSecretName" (dict "context" $) }}
