...
kind: Secret
metadata:
  name: (ternary $CERTIFICATE.name (if .context.Values.ingress (include "common.secrets.tlsSecretName" (dict "context" $context "hostname" $CERTIFICATE.hostname)) $CERTIFICATE.name))
